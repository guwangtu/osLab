...
{
    int c; /* becuase char is signed on aarch64 */
...
void eval(char* cmdline)
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        if ((pid = Fork()) == 0) { 	/* child runs user job */
            ...
            // set signal handlers to default handlers
...
/*
 * sigalrm_handler - The kernel sends a SIGALRM to the shell whenver the
 *    time period of foreground job ends.  Catch it and send SIGINT
 *    to the foreground job.
 */
void sigalrm_handler(int sig)
{
    ...
    /**
     * TBD: print timeout message and kill foreground process
     */
    static int t = 0;
    if (t == 0) {
        printf("timeout!alarm received\nJob[%d](%d) terminated by signal % d\n", jid, pid, SIGINT);
